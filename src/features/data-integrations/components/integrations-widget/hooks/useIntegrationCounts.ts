...
export function useIntegrationCounts() {
  ...
  // Source type IDs: 1=openshift, 2=amazon, 3=google, 4=azure (from seed data)
  const awsQuery = useSources({ sourceTypeIds: ['2'], limit: 1 });
  const googleQuery = useSources({ sourceTypeIds: ['3'], limit: 1 });
  const azureQuery = useSources({ sourceTypeIds: ['4'], limit: 1 });
  const openshiftQuery = useSources({ sourceTypeIds: ['1'], limit: 1 });
